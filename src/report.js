...
export function buildReport(state) {
  ...
  <table class="paper-table paper-signoffs"><colgroup><col><col><col></colgroup><tbody><tr>${[['Prepared by', r.preparedBy, r.preparedDate], ['Reviewed by', r.reviewedBy, r.reviewedDate], ['Approved by', r.approvedBy, r.approvedDate]].map(([label, name, date]) => `<td class="signature-cell"><b>${label} / signature</b><div class="signature-space"></div><div class="paper-text">${value(name)}</div><span>Date: ${dateLabel(date)}</span></td>`).join('')}</tr><tr><td class="paper-text remarks-cell"><b>Remarks from Rater/AO:</b><div>${value(r.raterRemarks)}</div></td><td class="paper-text remarks-cell"><b>Override CRR: ${value(r.override || 'None')}</b><div>${value(r.overrideReason)}</div><div><b>Authorized by:</b> ${value(r.overrideBy)}</div><small>No override for major document deficiencies.</small></td><td class="paper-text remarks-cell"><b>Remarks from Reviewer/Approver:</b><div>${value(r.reviewerRemarks)}</div></td></tr></tbody></table>`;
  ...
  return `<div class="paper-form paper-${state.type}">
    ...
    <div class="paper-policy"><p><b>Calculation / policy:</b> Section item mean × weight; sum contributions, round once to 2 decimals. Missing answers are not zero. ${escapeHtml(policy)}</p>${!result.policy.valid ? `<p><b>Weight total: ${escapeHtml(result.policy.sum ?? 'invalid')}%.</b> ${result.normalizedPreview !== null ? `Normalized preview: ${result.normalizedPreview.toFixed(2)}/10 (provisional; not a final CRR).` : ''}</p>` : ''}${result.policy.changed || state.policy.investmentB !== '' ? `<p><b>Bank interpretation recorded by:</b> ${value(state.policy.approvedBy)}. <b>Reference:</b> ${value(state.policy.reference)}.${state.policy.investmentB !== '' ? ` <b>SME item 23 B:</b> ${value(state.policy.investmentB)}.` : ''}</p>` : ''}${outstanding.length ? `<p><b>Outstanding items:</b> ${escapeHtml(outstanding.join(' '))}</p>` : ''}<p>Calculated rating is an assessment record, not a loan approval. Officer names are not authenticated electronic signatures.</p></div>
